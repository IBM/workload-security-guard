...
func ValueTests_Test(t *testing.T, profiles []ValueProfile, piles []ValuePile, configs []ValueConfig, arguments ...interface{}) {
	...
	t.Run("Basics", func(t *testing.T) {
		...
		// Initial Tests
		piles[9].Clear()
		piles[8].Merge(piles[4])
		configs[8].Fuse(configs[4])
...
func ValueTests_SimpleTest(t *testing.T, profiles []ValueProfile, piles []ValuePile, configs []ValueConfig, arguments ...interface{}) {
	...
	t.Run("Basics", func(t *testing.T) {
		...
		// test ConfigValue
		config.Learn(pile)
		if str := config.Decide(good); str != "" {
			t.Errorf("config.Decide(profile) wrong decision: %s", str)
		}
		if str := config.Decide(bad1); str == "" {
			t.Errorf("config.Decide(profile) expected a reject of %s after learning %s\n", args[1], args[0])
		}
		if str := config.Decide(bad2); str == "" {
			t.Errorf("config.Decide(profile) expected a reject of %s after learning %s\n", args[2], args[0])
		}
		configF.Fuse(config)
		if str := configF.Decide(good); str != "" {
			t.Errorf("config.Decide(profile) wrong decision: %s", str)
		}
		if str := configF.Decide(bad1); str == "" {
			t.Errorf("config.Decide(profile) expected a reject of %s after learning %s\n", args[1], args[0])
		}
		if str := configF.Decide(bad2); str == "" {
			t.Errorf("config.Decide(profile) expected a reject of %s after learning %s\n", args[2], args[0])
		}
		configM.Learn(pileM)
		fmt.Println(configM, good)
		if str := configM.Decide(good); str != "" {
			t.Errorf("config.Decide(profile) wrong decision: %s", str)
		}
		if str := configM.Decide(bad1); str == "" {
			t.Errorf("config.Decide(profile) expected a reject of %s after learning %s\n", args[1], args[0])
		}
		if str := configM.Decide(bad2); str == "" {
			t.Errorf("config.Decide(profile) expected a reject of %s after learning %s\n", args[2], args[0])
		}
