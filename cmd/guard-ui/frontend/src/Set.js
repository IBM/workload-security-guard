  ...
  console.log("Set dataVal", dataVal)
    ...
    function onSelectKey(k) {
        ...
        if (k !== "") {
          ...
          onDataChange(Object.keys(value))
          setData(Object.keys(value))
    ...
    //let keys = []
    for (let key of dataVal) {
        value[key] = true
       // keys.push(key)
    }
    console.log("Set value", value, "keys", dataVal)
    ...
    return (
       <TreeItem nodeId={nodeId} label={name}>
            ...
            <AddKeyDialog open={newkeyOpen} data ={dataVal} onClose={onNewKey} ></AddKeyDialog>
            <Paper sx={{
                    display: 'flex',
