...
function Req(props) {
  ...
  if (!data.mediatype) data.mediatype = []
  ...
  function handleMediaTypeChange(d) {
    data.mediatype = d
    console.log("handleMediaTypeChange", data)  
    onDataChange(data)
  };
...
return (
    <TreeItem nodeId={nodeId} label={name} sx={{ textAlign: "start"}}>
        ...
        <Set data={data.mediatype} nodeId={nodeId+">MediaType"} name="MediaType" onDataChange={handleMediaTypeChange}></Set>    
